...
class DownloadManager:
    ...
    async def download_playlist(
        ...
        for index, track in enumerate(added_tracks, 1):
            try:
                ...
                save_path = (
                    self.download_path / playlist_folder / artist_folder / album_folder
                )
                save_path.mkdir(parents=True, exist_ok=True)

                # Скачиваем трек
                print(
                    f"Загружаем трек {index}/{len(added_tracks)}: {track['title']} - {track['artist']}"
                )

                # Создаем callback для прогресса трека
                def track_progress_callback(
                    ...
                    try:
                        ...
                        db_manager.update_download_progress(
                            track["id"], int(progress_percent)
                        )
                    except Exception as e:
    ...
    async def download_track_async(
        ...
        try:
            ...
            file_path = self.client.download_track(track_id, str(save_path), quality)
